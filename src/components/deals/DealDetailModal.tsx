...
import { X, CalendarDays, Tag, AlignLeft, Users, Briefcase, DollarSign, Percent, LinkIcon, Edit2, Trash2, CheckCircle, AlertCircle, Clock, UserCircle as UserCircleIcon } from 'lucide-react';
...
const DealDetailModal: React.FC<DealDetailModalProps> = ({ isOpen, onClose, deal, getStageNameById, onEdit, onDelete }) => {
      ...
      <div className="bg-white rounded-xl shadow-2xl p-6 md:p-8 w-full max-w-2xl max-h-[90vh] overflow-y-auto scrollbar-thin scrollbar-thumb-slate-300 scrollbar-track-white relative" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="flex justify-between items-start mb-6 border-b border-slate-200 pb-4">
            ...
            <div className="flex space-x-2 flex-shrink-0 ml-4">
                {onEdit && (
                    <button 
                        onClick={() => onEdit(deal)} 
                        className="p-2 text-sm font-medium text-white bg-amber-500 hover:bg-amber-600 rounded-lg shadow-sm flex items-center transition-colors"
                ...
                {onDelete && (
                    <button 
                        ...
                        className="p-2 text-sm font-medium text-white bg-red-500 hover:bg-red-600 rounded-lg shadow-sm flex items-center transition-colors"
                        title="Deal löschen"
                    >
                        <Trash2 size={16} />
                    </button>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6 mb-6">
          ...
          <div>
            ...
            <p className="text-lg font-semibold text-slate-700">{getStageNameById(deal.stageId)}</p>
          </div>
          <div>
